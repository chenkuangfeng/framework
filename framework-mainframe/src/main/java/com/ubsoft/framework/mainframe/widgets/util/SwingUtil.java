...
public class SwingUtil
{
     ...
     * <code>putClientProperty(SwingUtilities2.AA_TEXT_PROPERTY_KEY, UIUtil.COMMON_AATEXT_INFO);</code><br>
     ...
     * @param root RootPaneContainer组件
     * @param action 需要执行的动作
     * @param keyName 键的名称，原则上可以是任意字符串，实际应用时建议使用有意义的字符串
     * @param keyCode 键的数字代码，比如KeyEvent.VK_T
     * @param modifiers 任意修饰符的按位或组合，比如Ctrl+Shift为KeyEvent.CTRL_MASK | KeyEvent.SHIFT_MASK
     */
     ...
     * @param root RootPaneContainer 窗体组件
     * @param defaultButton JButton 缺省按钮（按下Enter时响应）
     * @param escAction Action Escape事件（按下Esc时响应）
     */
    ...
    /**
     * 重置ToolTip的字体，在Ubuntu+Java6环境中，默认的Font.DIALOG会出现乱码，故需重置
     */
    public static void resetToolTipFont()
    {
        final String toolTipFontKey = "ToolTip.font";
        Object fontObject = UIManager.getDefaults().get(toolTipFontKey);

        if(fontObject instanceof Font)
        {
            Font font = (Font)fontObject;
            Font newFont = getFont(getDefaultFontName(), font.getStyle(), font.getSize());
            UIManager.getDefaults().put(toolTipFontKey, newFont);
        }
    }
     ...
     * @param image 源图像
     * @param angle 旋转角度
     * @param c 用于获取GraphicsConfiguration的组件
     * @return 旋转后的图像
     */
    ...
    /**
     * 创建数据布局和颜色模型与本机设备配置兼容的BufferedImage
     * @param width 宽度
     * @param height 高度
     * @param transparency 透明度模式
     * @param c 用于获取GraphicsConfiguration的组件
     * @return BufferedImage
     */
    public static BufferedImage createCompatibleBufferedImage(int width, int height, int transparency, Component c)
    {
        return getGraphicsConfiguration(c).createCompatibleImage(width, height, transparency);
    }
    ...
    {
        BufferedImage bufferedImage = getGraphicsConfiguration(c).createCompatibleImage(icon.getIconWidth(),
    ...
    public static String getDefaultFontName()
    {
        ...
        {
            String[] fontNames = {"Microsoft YaHei UI", "Microsoft YaHei", "SimSun", "WenQuanYi Micro Hei", "WenQuanYi Zen Hei"};
            GraphicsEnvironment ge = GraphicsEnvironment.getLocalGraphicsEnvironment();
            String[] availFontNames = ge.getAvailableFontFamilyNames(Locale.US);
            defaultFontName = Font.DIALOG;

            font: for(String avail: availFontNames)
            {
                for(String preferFont: fontNames)
                {
                    if(preferFont.equalsIgnoreCase(avail))
                    {
                        defaultFontName = preferFont;
                        break font;
                    }
                }
            }
    ...
    {
        return new Font(getDefaultFontName(), Font.PLAIN, 12);
    }

    /**
     * 获取字体，如果指定的字体名称不存在，则使用getDefaultFontName()
     * @param name 字体名称
     * @param style 字体样式
     * @param size 字体大小
     * @return 指定字体
    ...
    public static Font getFont(String name, int style, int size)
    {
        ...
        if(Util.searchFromArray(availFontNames, name) < 0)
        {
            availFontNames = ge.getAvailableFontFamilyNames(Locale.US);
    ...
    public static Font getTitleFont(boolean isCH)
    {
        ...
        if(Util.isWindows())
        {
            ...
            {
                font = getFont("\u5FAE\u8F6F\u96C5\u9ED1", Font.BOLD, isCH? 13: 12);
            }
            else
            {
                if(isCH)
                {
                    font = getFont("\u5B8B\u4F53", Font.BOLD, 13);
                }
                else
                {
                    font = getFont("Tahoma", Font.BOLD, 12);
                }
            }
        }
        else
        {
            font = new Font(getDefaultFontName(), Font.BOLD, isCH? 13: 12);
        }
     ...
     * @param imageInsets 图像的边框
     * @param paintRect 组件上需要绘制的区域
    ...
    public static void paintImage(Graphics g, Image image, Insets imageInsets, Rectangle paintRect, ImageObserver observer)
    {
        ...
        g2d.drawImage(image, imageLeft, imageTop, width - imageRight, height - imageBottom,
                        imageLeft, imageTop, imageWidth - imageRight, imageHeight - imageBottom, observer);
        g2d.drawImage(image, width - imageRight, imageTop, width, height - imageBottom,
                        imageWidth - imageRight, imageTop, imageWidth, imageHeight - imageBottom, observer);

        //Bottom
        g2d.drawImage(image, 0, height - imageBottom, imageLeft, height, 0, imageHeight - imageBottom, imageLeft, imageHeight, observer);
        g2d.drawImage(image, imageLeft, height - imageBottom, width - imageRight, height,
                        imageLeft, imageHeight - imageBottom, imageWidth - imageRight, imageHeight, observer);
        g2d.drawImage(image, width - imageRight, height - imageBottom, width, height,
                        imageWidth - imageRight, imageHeight - imageBottom, imageWidth, imageHeight, observer);
        g2d.translate(-x, -y);
     ...
     * @param disabledBackground 禁用时的背景颜色
     * @param image 背景图像
     * @param imageOnly 是否只显示图像
     * @param alpha 不透明度
     * @param visibleInsets 背景边框
     */
    public static void paintBackground(Graphics g, JComponent c, Color background, Color disabledBackground,
                    Image image, boolean imageOnly, float alpha, Insets visibleInsets)
    {
        if(alpha > 0.0 && !(imageOnly && image == null))
        {
            ...
            if(!imageOnly)
            {
                ...
                g2d.fillRect(visibleInsets.left, visibleInsets.top, c.getWidth() - visibleInsets.left - visibleInsets.right,
                                c.getHeight() - visibleInsets.top - visibleInsets.bottom);
            ...
            if(image != null)
            {
                Object oldHintValue = g2d.getRenderingHint(RenderingHints.KEY_ANTIALIASING);
                g2d.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
                g2d.drawImage(image, visibleInsets.left, visibleInsets.top, c.getWidth() - visibleInsets.left - visibleInsets.right,
                                c.getHeight() - visibleInsets.top - visibleInsets.bottom, c);
                g2d.setRenderingHint(RenderingHints.KEY_ANTIALIASING, oldHintValue);
            }
     ...
     * @param width 圆角矩形宽度
     * @param height 圆角矩形高度
     * @param cornerSize1 左上角的圆角大小
     * @param cornerSize2 右上角的圆角大小
     * @param cornerSize3 右下角的圆角大小
     * @param cornerSize4 左下角的圆角大小
     * @return 圆角矩形
     */
    public static Polygon createRoundRect(int x, int y, int width, int height, int cornerSize1, int cornerSize2,
                    int cornerSize3, int cornerSize4)
    {
        int nPoints = 8;
     ...
     * @param label 需要添加动作的JLabel
     * @param action 单击左键时要执行的动作
     */
    ...
    public static Color[] createRandomColors(int count)
    {
        ...
        while(set.size() < count)
        {
            ...
            while(Math.abs(indexR - indexG) < sameFactor && Math.abs(indexG - indexB) < sameFactor
                            && Math.abs(indexB - indexR) < sameFactor)
            {
